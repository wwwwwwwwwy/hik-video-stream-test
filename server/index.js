...
function connectUpstream(ws, state, record) {
  const upstream = new UpstreamWebSocket(record.upstreamConnectUrl);
  ...
  upstream.addEventListener('message', (event) => {
    const payload = normalizeUpstreamPlaybackMessage(event.data, event.data instanceof ArrayBuffer);
    if (debugProxy) {
      console.log('[proxy] upstream -> client', { payload: describePayload(payload) });
    }
  ...
  upstream.addEventListener('error', (event) => {
    if (debugProxy) {
      console.log('[proxy] upstream error', event.error?.message || 'unknown upstream error');
    }
  ...
  upstream.addEventListener('close', (event) => {
    if (debugProxy) {
      console.log('[proxy] upstream close', event.code, event.reason);
    }
...
server.on('upgrade', (req, socket, head) => {
  const requestUrl = new URL(req.url || '/', `http://${req.headers.host || `${host}:${port}`}`);
  const proxyId = requestUrl.searchParams.get('sessionID') || requestUrl.searchParams.get('session') || null;
...
wss.on('connection', (ws) => {
  ...
  if (debugProxy) {
    console.log('[proxy] client open', {
      id: record.id,
      upstreamConnectUrl: record.upstreamConnectUrl,
      upstreamPlayURL: record.upstreamPlayURL
    });
  }
  ...
  ws.on('close', (code, reason) => {
    ...
    const currentState = bridgeStates.get(ws);
    if (currentState?.upstream && currentState.upstream.readyState === UpstreamWebSocket.OPEN) {
